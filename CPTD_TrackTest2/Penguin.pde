class Penguin {
  ...
  void calculateSlope() {
    for (int i = 0; i < puffle.length; i++) {   
      if (dist(pengx, pengy, puffle[i].getX(), puffle[i].getY()) < 60) {
        slopeX = 0.07 * (puffle[i].getX() - pengx);
        slopeXConstrain = constrain(slopeX, -6, 6);

        slopeY = 0.07 * (puffle[i].getY() - pengy);
  ...
  float getSlopeXConst() {
    return slopeXConstrain;
  }
  float getSlopeYConst() {
    return slopeYConstrain;
  }
